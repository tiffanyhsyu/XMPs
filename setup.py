...
#
setup_keywords['name'] = 'xmps'
setup_keywords['description'] = 'ArcLine Database'
setup_keywords['author'] = 'Tiffany Hsyu'
setup_keywords['author_email'] = 'tiffany@ucolick.org'
setup_keywords['license'] = 'BSD'
setup_keywords['url'] = 'https://github.com/tiffanyhsyu/xmps'
#
...
setup_keywords['packages'] = ['xmps'] #find_packages('pypit')
...
for pyx_file in pyx_files:
    ...
    ext = Extension('xmps.'+pyx_split2[1], [pyx_file],
...
data_generator = os.walk('xmps/data')
...
#settings = glob.glob('xmps/settings/settings.*')
#settings = ['/'.join(path.split('/')[1:]) for path in settings]
#data_files.extend(settings)
setup_keywords['package_data'] = {'xmps': data_files,
                                  '': ['*.rst', '*.txt']}
