...
def mags_from_spec(file_list,z_list):
    ...
    for k in file_list:
        ...
        for rs in z_list:
           ...
           #you can load your filters here if they are available in the speclite package.
            ...
            ##Example
            g.append((mags['sdss2010-g'])[0])  #values at a specific redshift
            r.append((mags['sdss2010-r'])[0])
            i.append((mags['sdss2010-i'])[0])
        print g,r,i
        return g,r,i  # mags for each file for all redshifts
...
#Filter curves here
u = Table.read('/Users/admin/Dropbox/u.dat.txt', format='ascii')  # , delimiter=' ', guess=False)
g = Table.read('/Users/admin/Dropbox/g.dat.txt', format='ascii')  # , delimiter=' ', guess=False)
r = Table.read('/Users/admin/Dropbox/r.dat.txt', format='ascii')  # , delimiter=' ', guess=False)
i = Table.read('/Users/admin/Dropbox/i.dat.txt', format='ascii')  # , delimiter=' ', guess=False)
z = Table.read('/Users/admin/Dropbox/z.dat.txt', format='ascii')  # , delimiter=' ', guess=False)
...
def mag_from_spec2(file_list,z_list):
    ...
    for file in file_list:
        ...
        for rs in z_list:
            ...
            mags = []
            for c in filters:
                ind1 = np.where(wave == fil(c,wave)[0])[0][0]
                ind2 = np.where(wave == fil(c,wave)[1])[0][0]
                wave = wave[ind1:ind2]
                flux = flux[ind1:ind2]
                ...
                m = (-2.5 / np.log(10)) * (np.arcsinh((f / f_zero) / (2 * fil(c,wave)[2])) + np.log(fil(c,wave)[2]))
            ...
            print u_g
            return u_g,g_r,r_i #return the colors for each redshift
